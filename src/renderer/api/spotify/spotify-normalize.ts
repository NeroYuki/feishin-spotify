...
export const normalizeSpotifyArtist = (artist: SpotifyFullArtist): AlbumArtist => ({
...
export const normalizeSpotifyAlbum = (album: SpotifyFullAlbum): Album => ({
    _itemType: LibraryItem.ALBUM,
    _serverId: SPOTIFY_SERVER_ID,
    _serverType: ServerType.SPOTIFY,
    albumArtistName: album.artists?.[0]?.name ?? '',
    albumArtists: (album.artists ?? []).map(normalizeArtist),
    ...
    isCompilation: album.album_type === 'compilation',
    lastPlayedAt: null,
    mbzId: null,
    mbzReleaseGroupId: null,
    name: album.name,
    originalDate: null,
    originalYear: parseInt(album.release_date?.substring(0, 4) ?? '0', 10),
    participants: null,
    playCount: null,
    recordLabels: [],
    ...
    releaseTypes: [],
    releaseYear: parseInt(album.release_date?.substring(0, 4) ?? '0', 10),
    size: null,
...
/**
 * Normalizes a simplified album track (which lacks full album context).
 * The caller must provide the parent album to fill in album-level fields.
 */
export const normalizeSpotifyAlbumTrack = (
    track: SpotifySimplifiedTrack,
    album: SpotifyFullAlbum,
): Song => {
...
/**
 * Normalizes a simplified album (e.g. from search results) to the domain Album type.
 */
export const normalizeSpotifySimplifiedAlbum = (album: SpotifySimplifiedAlbum): Album => ({
    _itemType: LibraryItem.ALBUM,
    _serverId: SPOTIFY_SERVER_ID,
    _serverType: ServerType.SPOTIFY,
    albumArtistName: album.artists?.[0]?.name ?? '',
    ...
    releaseYear: parseInt(album.release_date?.substring(0, 4) ?? '0', 10),
    size: null,
    songCount: album.total_tracks ?? null,
    sortName: album.name,
    tags: null,
    updatedAt: '',
    userFavorite: false,
...
/**
 * Normalizes a search-result artist (has images + genres but no followers field).
 */
export const normalizeSpotifySearchArtist = (artist: SpotifySearchArtist): AlbumArtist => ({
    _itemType: LibraryItem.ALBUM_ARTIST,
    _serverId: SPOTIFY_SERVER_ID,
    _serverType: ServerType.SPOTIFY,
    albumCount: null,
